...
public class LaptopFrame extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LaptopFrame frame = new LaptopFrame();
	...
	public LaptopFrame() {
		...
		lblNewLabel = new JLabel("LAPTOP PAGE");
	...
	private Product getProductAtRow(int row) {
	    ...
	    String description = productModel.findDescription(brand, modelValue);
	...
	private void showProductDetails(Product product) {
		...
		imgLabel.setBounds(364, 68, 200, 204);
		panel.add(imgLabel);
	...
	private void loadData() {
		...
		defaultTableModel.addColumn("Price (in $)");
		defaultTableModel.addColumn("Photo");
		defaultTableModel.addColumn("Category");
		for (Product product : productModel.findAll3()) { //change here
